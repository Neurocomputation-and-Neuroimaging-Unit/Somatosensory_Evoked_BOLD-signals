...
% par_addr = hex2dec('3FF8'); % in Hex
% par_addr = 16376; % in Dec
ioObj = io64;
status = io64(ioObj);
...
DesignMat = [[1:(trials+this_catch)]'   ...     %1. Trial Number
    stim_vol' ...                               %2. Stimulated volume (new!)
    slice_ind' ...                              %3. Stimulated Slice in stim_vol (new!)
        ...
        if keyCode == fmriTrig1 ||  keyCode == fmriTrig2
            ...
            elseif vol_count == 1
                ...
                if abs(actual_tr - expected_tr) > 0.75 % very random value that should be foolproof 
                    error('wrong sequence! TRs dont match.'); % to make sure we don't use any other (the old) sequence by mistake
                end
            end
...
% plot to see whether thats the case: only "dip" in the line should be the
% catch trial! any other dips mean a major fuck-up by matlab!
plot(result_time);
hold on
